...
public class MyClass {
    public static boolean rightReccursion(MyLang testLang) {
        int[] lookFor = new int[testLang.getNonTerminals().length];
        int[] NonTerminals = testLang.getNonTerminals();
        int[] epsilonNonterminals = testLang.getEpsilonNonterminals();

        boolean isRightRec = false;
        boolean isRightRec1 = false;

        for (int i = 0; i < NonTerminals.length; ++i) {
            int count = 0;
            int count1 = 1;
            isRightRec = false;
            lookFor[count] = NonTerminals[i];

            do {
                Iterator iterator = testLang.getLanguarge().iterator();
                ...
                while (true) {
                    ...
                    do {
                        ...
                        JavaTeacherLib.Node tmp = (JavaTeacherLib.Node) iterator.next();
                        rool = tmp.getRoole();
                    } while (rool[0] != lookFor[count]);


                    int j;
                    for (j = rool.length - 1; j > 0 && rool[j] <= 0 && rool[j] != lookFor[0]; --j) {
                        int k;

                        k = 0;
                        while (k < count1 && rool[j] != lookFor[k]) {
                            ++k;
                        }

                        if (k == count1) {
                            lookFor[count1++] = rool[k];
                        }

                        if (epsilonNonterminals == null) {
                            break;
                        }

                        k = 0;
                        while (k < epsilonNonterminals.length && rool[j] != epsilonNonterminals[k]) {
                            ++k;
                        }

                        if (k == epsilonNonterminals.length) {
                            break;
                        }
                    }

                    if (j != 0 && rool[j] == lookFor[0]) {
                        System.out.print("Праворекрусивний нетермінал це: " + testLang.getLexemaText(lookFor[0]) + "\n");
                        isRightRec = true;
                        isRightRec1 = true;
                        break;
                    }
